...
import { get_project, event_insert, rollup_tables_insert } from "./repository";
...
const store = async (
    ...
    const event = await event_insert(c.env.DB, rows);
    c.executionCtx.waitUntil(rollup_tables_insert(c.env.DB, rows));
    return event;
};
